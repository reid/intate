...
var EXTRACTOR_TABLE = "https://gist.github.com/raw/410824/7a98a4e9dbfc55f3b8363f3964e935609162638e/extract-etsy.xml";
...
get("/yap", function () {
    this.render("yap/yap.html.haml", {
        layout : "yap/layout.html.haml"
    });
});

get("/yap/friend", function () {
    var express = this;
    if ("selectedFriend" in express.params.get) {
        return express.render("yap/friend.html.haml", {
            layout : "yap/layout.html.haml",
            locals : {
                guid : express.params.get.selectedFriend,
                jscript : require("fs").readFileSync(__dirname + "/views/yap/app.js")
            }
        });
    }
    express.redirect("/yap");
});
...
function superYQL (express, likes) {
    ...
    var context = likes.join(" ").replace(/[^a-zA-Z0-9 ]+/g, ""),
        query = "use \"" + EXTRACTOR_TABLE + "\" as extractor;"
        + " select * from extractor where context = \"" + context + "\""
        + " and api_key = \"" + process.env.ETSY_KEY + "\""
    yql(query).addListener("response", function (listings) {
        console.error("YQL: " + query);
        try {
            listings = JSON.parse(listings).query.results.etsy.results;
        } catch (ex) {
            console.error("Failed to parse listings from YQL: " + listings);
            listings = false;
        }
...
get("/api/yahoo/etsy/:id", function (guid) {
    ...
    oapi.makeRequest(
        ...
        [
            "q=select%20%2A%20from%20social.updates%20where%20guid%20%3D%20%22" + guid + "%22",
            "format=json"
        ],
        function (err, data, res) {
            if (err) return express.respond(500, "YQL signed error: " + err);
            data = JSON.parse(data);
            if (!data) return express.respond(500, "YQL failed to respond properly");
            //var s = require("sys");
            //s.debug(s.inspect(data.query.results.update));
            var profile = data.query.results.update;
            var likes = [];
            profile.forEach(function (like) {
                // todo: discard all before first Capital letter
                likes.push(like.title);
            });
            //s.debug(s.inspect(likes));
            if (!likes.length) return express.respond(204);
            superYQL(express, likes);
       }
    );
